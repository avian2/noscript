...
const IC_EVAL_PATTERN = fuzzify('eval|set(?:Timeout|Interval)|[fF]unction|Script|toString') + IC_WINDOW_OPENER_PATTERN;
const IC_EVENT_PATTERN = fuzzify("on(?:key|row|mouse|drag|befor|after|focus|move|data|select|end|cut|load|zoom|help|stop|drop|blur|copy|paste|click|begin|error|input|abort|start|reset|resize|change|bounce|unload|finish|scroll|submit|repeat|message|invalid|activate|dblclick|forminput|deactivate|formchange|cellchange|errorupdate|losecapture|contextmenu|filterchange|controlselect|propertychange|readystatechange)"
  ) + "(?:\\W*[a-z])*"; // generated by html5_events_gen, see http://mxr.mozilla.org/mozilla-central/source/parser/html/nsHtml5AtomList.h
...
var InjectionChecker = {
  ...
  _singleAssignmentRx: new RegExp(
    "(?:\\b" + fuzzify('document') + "\\b[\\s\\S]*\\.|\\s" + fuzzify('setter') + "\\b[\\s\\S]*=)|/.*/[\\s\\S]*\\." + fuzzify('source') + "|"
...
XSanitizer.prototype = {
  ...
  _brutalReplRx: new RegExp(
    '(?:' + fuzzify('setter|location|cookie|name|document|toString|') +
