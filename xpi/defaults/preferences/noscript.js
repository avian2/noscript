...
pref("noscript.surrogate.adfly.replacement", "if(/\\bblocked\\b/.test(location.href))history.back();else if(location.hash){stop();open(decodeURIComponent(location.hash.substring(1)),'_self')} else for each(let s in document.getElementsByTagName('script')){let m=s.textContent.match(/\\bcountdown\\b[\\s\\S]+\\bvar\\s+url\\s*=\\s*[\"']([^'\"]+)/);if(m){open(document.getElementsByTagName('iframe')[0].src.replace(/&t=[^&]*/,'&t=0')+'&_tt='+Date.now()+'#'+encodeURIComponent(m[1]),'_self');break}}");
